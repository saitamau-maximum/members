...
新しくできた `members` ディレクトリの中に、 `data` というディレクトリがあります。
...
ファイルのパスを指定してあげる必要があるので、 `README.md` があるディレクトリで操作した場合は `data/username.json` としてください。
...
毎年 4 月ごろ、メンバー情報の `isActive` が `false` になります。
すなわち、一時的に退会した、という扱いになります。
継続して活動を希望する場合には、上記「入会希望者のやること」と同様の手順を行ってください。

なお、すでにこのリポジトリへの書き込み権限がある (はず) なので、 Fork せずに、直接このリポジトリにブランチを作成して変更を加えることもできます。

---

## 情報に変更が生じたときに行うこと

メンバー情報に変更が生じた場合も、上と同様の手順で変更を加えることができます。
